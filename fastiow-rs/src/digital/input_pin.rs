...
use crate::internal::{
    disable_gpio, enable_gpio, read_report_non_blocking, IOWarriorData, IOWarriorMutData, PinType,
    Pipe,
};
use crate::{PinError, PinSetupError};
...
impl InputPin {
    pub(crate) fn new(
    ...
    ) -> Result<InputPin, PinSetupError> {
        {
            ...
            enable_gpio(&data, &mut mut_data, PinType::Input, pin)?;
        }
